...
pfe::Camera::Camera(sf::Angle fov, int rays, float lenght, float stepRay)
    : FOV(fov), numRays(rays), rayLenght(lenght), rayStep(stepRay)
{
    cameraView.setPrimitiveType(sf::PrimitiveType::Lines);
    minimapView.setPrimitiveType(sf::PrimitiveType::Lines);
}

void pfe::Camera::UpdateInfo(sf::Vector2f position, sf::Angle rotation)
{
    this->position = position;
    this->rotation = rotation;
}

void pfe::Camera::DrawMinimapView(sf::RenderWindow& window)
{
    if (minimapView.getVertexCount() > 0)
        window.draw(minimapView);
}

void pfe::Camera::DrawCameraView(sf::RenderWindow& window)
{
    if (cameraView.getVertexCount() > 0)
        window.draw(cameraView);
}

void pfe::Camera::Render(const sf::Vector2u& windowSize, const pfe::Map& map)
{
    minimapView.clear();
    cameraView.clear();
...
void pfe::Camera::castRayDefault(const sf::Angle& direction, int iRay, const sf::Vector2u& windowSize, const pfe::Map& map)
{
    ...
    for (; lenght < rayLenght; lenght += rayStep) {
...
void pfe::Camera::castRaySectionMethod(const sf::Angle& direction, int iRay, const sf::Vector2u& windowSize, const pfe::Map& map)
{
...
void pfe::Camera::drawLine(const float& lenght, const pfe::Map::Wall& wall, const int& iRay, const sf::Vector2u& windowSize)
{
